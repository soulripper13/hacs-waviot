# coordinator.py - Fetches WAVIoT modem data from Jan 1 last year up to now
import aiohttp
...
class WaviotDataUpdateCoordinator(DataUpdateCoordinator):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                ...
                url = (
                    ...
                    f"&start={int(prev_year_start.timestamp())}&end={int(now.timestamp())}"
                )
                _LOGGER.debug("Fetching readings from Jan 1 last year: %s", url)

                readings = {}
                async with session.get(url) as resp:
                    ch_data = await resp.json()
                    values = ch_data.get("values", {}) if ch_data else {}
                    _LOGGER.debug("Raw API values count: %s", len(values))
                    ...
                    for ts, val in values.items():
                        try:
                            ...
                            if ts_sec > 1e12:  # milliseconds → seconds
                                ts_sec //= 1000
                            readings[ts_sec] = float(val)
                        except Exception as ex:
                            _LOGGER.warning("Skipping invalid reading ts=%s val=%s (%s)", ts, val, ex)

                # Sort by timestamp
                self.data["readings"] = dict(sorted(readings.items()))
                _LOGGER.debug("Processed readings count: %s", len(self.data["readings"]))
    ...
    def _compute_usage(self):
        ...
        latest_value = readings[latest_ts]
        ...
        # Hourly usage
        hourly_val = max(
            (v for ts, v in readings.items() if datetime.fromtimestamp(ts, tz=timezone.utc) <= one_hour_ago),
            default=None
        )
        ...
        # Daily usage
        daily_val = max(
            (v for ts, v in readings.items() if datetime.fromtimestamp(ts, tz=timezone.utc) <= one_day_ago),
            default=None
        )
